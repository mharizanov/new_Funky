...
void loop() {
  ...
  if(got_interrupt) {  //How did we get here? Thru interrupt or wait expired?)
    temptx.temp=1;
    rfwrite(); // Send data via RF   
  }
  else {
    temptx.temp=0;  // Set the alarm status to off
    rfwrite(); // Send data via RF     
  }
  
//  Sleepy::loseSomeTime(60000);  //sleep some more to debounce
...
*/
  ...
  for(int j = 1; ((j < 720) && (got_interrupt==false)); j++) {    // Sleep for 30 minutes
